...
/*!***********************!*\
  !*** ./src/Script.js ***!
  \***********************/
/*! exports provided: default, readColorValue, setColorValueWithLayers, setColorValueWithCurrentLayer, restoreLayer, ungroupWithLayer */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "default", function() { return onRun; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "readColorValue", function() { return readColorValue; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "setColorValueWithLayers", function() { return setColorValueWithLayers; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "setColorValueWithCurrentLayer", function() { return setColorValueWithCurrentLayer; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "restoreLayer", function() { return restoreLayer; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "ungroupWithLayer", function() { return ungroupWithLayer; });
...
var BorderPositions = ["center", "inside", "outside"],
    ...
    ResizingType = ["stretch", "corner", "resize", "float"]; // 字体色库
...
var bgLineColors = ["bgline_color_b, 细线分割线(全端分割线)", "bgline_color_c, 组、面分割(此颜色慎用)", "bgline_color_d, 产品底层背景色", "bgline_color_e, 内容层及卡片背景", "bgline_color_f, 分享、发布模态层背景(如模态层上有白色图形时)", "bgline_color_g, 内容层及卡片上子视图背景(楼中楼、分享贴底色)", "bgline_color_h, 悬浮视图层背景(内容层加阴影的视图)", "bgline_color_i, 模态层弹窗类上子视图背景", "bgline_color_j, 输入类容器背景", "bgline_color_k, 模态层弹窗类背景(筛选、更多)"];
var otherColors = ["cont_color_h", "linktip_color_a", "linktip_color_b", "linktip_color_c", "linktip_color_d", "linktip_color_e", "other_color_a", "other_color_b", "other_color_c", "other_color_f", "other_color_g"];
...
var shadowColors = ["shadow_color_a0", "shadow_color_a0.08", "shadow_color_a0.16", "shadow_color_a0.25", "shadow_color_a0.33", "shadow_color_a0.42"];
...
var app = NSApplication.sharedApplication(),
    ...
    // MSDocument object,
selection,
    // an NSArray of the layer(s) that are selected in the current document
plugin,
    command,
    page,
    // the current page in the current document
artboards,
    // artboards and slices on the current page
selectedArtboard,
    currentLayer; //--------------------------------------//
//               Context                //
//--------------------------------------//
...
function initContext(context) {
  doc = context.document, selection = context.selection, plugin = context.plugin, command = context.command, page = doc.currentPage(), artboards = page.artboards(), // 只读
  selectedArtboard = page.currentArtboard(), currentLayer = selection.count() > 0 ? selection[0] : undefined; // 当前画板默认是用户选择的第一个画板
} // 读取一个图层的色值
...
var setColorValueAllLayers = function setColorValueAllLayers(context) {
  // 初始化上下文
  initContext(context);
...
function setColorValueWithLayers(context) {
  // 初始化上下文
  initContext(context);
  setEachLayerName(currentLayer);
}

var setEachLayerName = function setEachLayerName(layer) {
  var finalLayer = getChildren(layer);
...
var setColorValueWithOneLayer = function setColorValueWithOneLayer(context, layer) {
  // 初始化上下文
  initContext(context);
...
var setColorValueWithSectionLayers = function setColorValueWithSectionLayers(context, selection) {
  // 初始化上下文
  initContext(context);
  openSetColorPanelWithLayers(context, selection);
};

function setColorValueWithCurrentLayer(context) {
  initContext(context);
...
var setSelectedLayerColorValue = function setSelectedLayerColorValue(context) {
  // 初始化上下文
  initContext(context);
};

function restoreLayer(context) {
  // 初始化上下文
  initContext(context);
  restorSelectionLayerName(selection);
}

var restorEachLayerName = function restorEachLayerName(layer) {
  var finalLayer = getChildren(layer);
...
var restorSelectionLayerName = function restorSelectionLayerName(selection) {
...
var ungroupAllLayer = function ungroupAllLayer(context) {
  // 初始化上下文
  initContext(context);
...
var getColor = function getColor(layer) {
  var text = '';
...
var exportText = function exportText(layer) {
  var returnText = [];
...
var exportSize = function exportSize(layer) {
...
var getFills = function getFills(style) {
  var fillsData = [],
      fill,
...
var getBorders = function getBorders(style) {
  var bordersData = [],
      border,
...
var colorToKey = function colorToKey(color) {
...
var colorToKeyForText = function colorToKeyForText(color) {
  var colorHex = colorToJSON(color);
  var colorArray = [];
  var key = colorHex.color + "_" + colorHex.alpha;
...
var colorToKeyForBorder = function colorToKeyForBorder(color) {
  var colorHex = colorToJSON(color);
  var colorArray = [];
  var key = colorHex.color + "_" + colorHex.alpha;
...
function openSetColorPanel(context, layer) {
  ...
  var view = NSView.alloc().initWithFrame(NSMakeRect(0, 0, 400, 150)); // Label
  ...
  function buttonAction(button) {
    ...
    var value = tempValue;
    command.setValue_forKey_onLayer(value, kLayerTBColorCustomKey, currentLayer); // 将当前的layer的名称替换成颜色名。
...
function openSetColorPanelWithLayers(context, selection) {
  ...
  restoreButton.setCOSJSTargetFunction(function (sender) {
    ...
    for (var _key in selection) {
      if (selection.hasOwnProperty(_key)) {
        var layer = selection[_key];
  ...
  function buttonAction(button) {
    ...
    for (var i = 0; i < selection.count(); i++) {
      var layer = selection[i];
      command.setValue_forKey_onLayer(value, kLayerTBColorCustomKey, layer); // 将当前的layer的名称替换成颜色名。
